...
public class ZS_RXTX_SerialReader extends Observable implements Runnable,SerialPortEventListener
    {
    static CommPortIdentifier portId;
    int delayRead = 100; // 端口读入数据事件触发后,等待delayRead毫秒后再读取,以便让数据一次性读完
    int numBytes; // buffer中的实际数据字节数
    private static byte[] readBuffer = new byte[1024]; // 4k的buffer空间,缓存串口读入的数据
    private static byte[] messageByte = new byte[1024];//用来清空readBuffer
   ...
   public void run(byte[] byt) {
	   	try { 
	   	 ...
	   	 try {
	   		 ...
	   		 { 	 
	   			 System.out.println("run message:"+byt);
	   	        outputStream.write(byt); //往串口发送数据，是双向通讯的。
	   		 }
	   	} catch (IOException e) {}
	   } 
...
//                    numBytes = inputStream.read( readBuffer );
    ...
    public void openSerialPort(String message)
    {
        ...
        params.put( ZS_RXTX_SerialReader.PARAMS_TIMEOUT, 100 ); // 设备超时时间 1秒
